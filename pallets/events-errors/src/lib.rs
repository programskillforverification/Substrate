...
pub mod pallet {
	...
	pub enum Event<T: Config> {
		SetClass { class: u32, who: T::AccountId },
		SetStudentInfo { student_number: u32, student_name: u128, who: T::AccountId },
		SetDormInfo { dorm_number: u32, bed_number: u32, student_number: u32, who: T::AccountId },
	}
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn set_class_info(origin: OriginFor<T>, class: u32) -> DispatchResult {
			...
			// Emit an event.
			Self::deposit_event(Event::SetClass { class, who });
		...
		pub fn set_student_info(
			...
			if !StudentInfo::<T>::contains_key(student_number) {
				...
				Self::deposit_event(Event::SetStudentInfo { student_number, student_name, who });
				Ok(())
			} else {
